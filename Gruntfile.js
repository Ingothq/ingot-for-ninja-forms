    ...
    grunt.initConfig({
        ...
        gitcommit: {
            commit: {
                ...
                files: {
                    src: [ 'package.json', 'readme.txt', 'nf-stripe.php', 'releases/<%= pkg.name %>-<%= pkg.version %>.zip' ]
                }
            }
        },
        gitpush: {
        ...
        replace: {
            core_file: {
                src: [ 'ingot-for-ninja-forms.php' ],
                overwrite: true,
                replacements: [{
                    from: /Version:\s*(.*)/,
                    to: "Version: <%= pkg.version %>"
                }, {
                    from: /define\(\s*'INGOT_nf_VER',\s*'(.*)'\s*\);/,
                    to: "define( 'INGOT_nf_VER', '<%= pkg.version %>' );"
                }]
            }
        }
